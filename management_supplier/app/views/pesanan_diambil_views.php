...
<div class="container-fluid mt-4">
    <h3 class="mb-3">Pesanan Diambil</h3>
    ...
    <div class="panel panel-primary mb-4 shadow-sm">
        ...
        <div class="panel-body p-3">
            <form action="index.php?controller=pesanan_diambil&action=simpan" method="post">
                <div class="row mb-3">
                    <div class="col-md-6">
                    ...
                    <tbody>
                        <?php foreach($data as $row){ ?>
                            <tr>
                                ...
                                <td><?= $row['status']; ?></td>
                                <td>
                                    <a href="index.php?controller=pesanan_diambil&action=detail&id=<?= $row['id_pesanan']; ?>" class="btn btn-info btn-sm">Detail</a>
                                    <a href="index.php?controller=pesanan_diambil&action=edit&id=<?= $row['id_pesanan']; ?>" class="btn btn-warning btn-sm">Edit</a>
                                    <?php if($row['status'] != 'Selesai'): ?>
                                        <a href="index.php?controller=pesanan_diambil&action=selesai&id=<?= $row['id_pesanan']; ?>" class="btn btn-success btn-sm" onclick="return confirm('Selesaikan pesanan?')">Selesai</a>
                                    <?php endif; ?>
                                    <a href="index.php?controller=pesanan_diambil&action=hapus&id=<?= $row['id_pesanan']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Hapus pesanan?')">Hapus</a>
                                </td>
                            </tr>
                        <?php } ?>
                    </tbody>
...
<?php
// Buat array JS dari PHP supaya select barang baru bisa di-clone
$barangsArray = [];
