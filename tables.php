...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "INSERT INTO barang (nama_barang, stok, harga, kondisi_barang, gambar)
              VALUES ('$nama', '$stok', '$harga', '$kondisi', '$gambar')";
...
if (isset($_GET['hapus'])) {
    ...
    if (!empty($row['gambar']) && file_exists(__DIR__ . "/image/" . $row['gambar'])) {
        unlink(__DIR__ . "/image/" . $row['gambar']);
    }
                                        ...
                                        <?php if (!empty($dataBarang)): ?>
                                            <?php foreach ($dataBarang as $row): ?>
                                                <tr>
                                                    ...
                                                    <td><?= ucfirst($row['kondisi_barang']); ?></td>

                                                    <!-- kolom gambar -->
                                                   <td>
                                                        <?php
                                                        $gambar = isset($row['Gambar']) ? $row['Gambar'] : ''; 
                                                        $gambarPath = "http://localhost/management_supplier/image/" . rawurlencode($gambar);
                                                        ?>
                                                        <?php if (!empty($gambar)): ?>
                                                            <img src="<?= $gambarPath; ?>" 
                                                                alt="<?= htmlspecialchars($row['nama_barang']); ?>" 
                                                                width="80" height="80">
                                                            <small><?= htmlspecialchars($gambar); ?></small>
                                                        <?php else: ?>
                                                    ...
                                                    <td>
                                                        ...
                                                        <a href="hapus_barang.php?id=<?= $row['id_barang']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Yakin mau hapus data ini?')">Hapus</a>
                                                    </td>
                                                </tr>
                                            <?php endforeach; ?>
                                        <?php else: ?>
